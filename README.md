...
En ambos casos, una vez lanzada la aplicación se debe acceder mediante el navegador indicando la dirección **localhost:3000**.
...
El contenido actual de Registro está formado por cinco strings (“antecedentes médicos”, “analítica sanguínea”, “enfermedades crónicas”, “anotaciones subjetivas” y “medicación recetada”) orientados al caso práctico que se plantea en la demo. 

**2. Función annadirRegistroEnCajaFuerte**:
El objetivo es añadir un nuevo registro dentro de la caja fuerte, de tal manera que cuando un paciente vaya a cualquier hospital, se cree y almacene sobre la blockchain un nuevo registro sobre la información que puede haber aportado dicha intervención. Para poder llevar a cabo la demo se han incluido los parámetros acordes a un ingreso en urgencias por hiperglucemia, y posteriormente hipoglucemia, además del SBT necesario para aplicar la función modifier. 

Más concretamente, estos son antecedentesMedicos, analiticaSanguinea, enfermedadesCronicas, anotacionesSubjetivas y medicacionRecetada. En función de la información que se quiera almacenar se incluirán unos parámetros u otros, pero deben mantener esa correlación con la estructura. Por otro lado, esta no devuelve nada.  
...
**3. Funciones recuperarRegistro y recuperarVault**: 
Ambas tienen el mismo objetivo, recuperar el o los registros almacenados a partir de una clave (el identificador tokenId de un SBT). La primera de ellas permite obtener uno en concreto, mientras que la segunda devuelve todos para una clave dada. Es importante mencionar que en la aplicación web esta función devuelve un valor distinto a si se llama directamente por contrato. En la web, esta muestra una lista con la cantidad de registros disponibles, de tal forma que el usuario seleccione mediante recuperarRegistro aquel que le interese. Se ha desarrollado esta diferencia por motivos estéticos, ya que teniendo muchos registros almacenados, la visualización del contenido mediante recuperarVault sería engorrosa y poco práctica.

RecuperarRegistro cuenta con dos parámetros de entrada, uint256 index y uint256 _soulBoundToken. El primero de ellos se emplea para señalar el registro en concreto que se quiere recuperar, siendo 0 el primero en haberse almacenado, por lo que el índice más alto será el registro más reciente. El segundo, en cambio, sirve para poder pasar el identificador del SBT como parámetro del modifier hasSBT. Esta función devuelve el contenido del objeto Registro que se ha especificado mediante el índice introducido por parámetro.

Respecto a la lógica interna de recuperarRegistro, la primera línea establece una condición para poder ejecutar la función. Si la longitud de la lista que representa el valor del diccionario cajaFuerteRegistros vinculado a un SBT dado no es mayor a cero, automáticamente cancela la ejecución y salta un mensaje de aviso. Tras dicha comprobación se crea una variable para recuperar el Registro especificado mediante el índice de la lista de Registros del diccionario y se devuelve el contenido del mismo.

Adicionalmente, esta función está condicionada por el modifier hasSBT(_soulBoundToken) que exige que quien llama la función para leer un registro vinculado a un SBT deba ser el poseedor de dicho SBT (lo tenga en la dirección desde la que llama a la función) o se le hayan otorgado permisos mediante la función trustYourFam para su administración. Para comprobar los permisos se hace uso de la función amIFam del contrato de los SBT.


**4. El modifier hasSBT**:
En este contrato se cuenta también con un modifier hasSBT(_soulBoundToken). Se exige que quien llama a la función a la que este se aplica, tanto para leer como escribir un registro vinculado a un SBT, deba ser el poseedor de dicho SBT (lo tenga en la dirección desde la que llama a la función) o se le hayan otorgado permisos mediante la función trustYourFam para su administración. Para comprobar los permisos se hace uso de la función amIFam del contrato de los SBT. Además, al igual que el modificador del contrato SBT, es necesario que el tokenId que devuelve la función walletOfOwner sea distinto de cero.

La lógica “_;”, en la sintaxis de Solidity, establece que primero se ejecutará la condición y posteriormente el resto del código al que se aplique el modifier.

Como se ha podido observar en los puntos anteriores, todas las funciones de este código hacen uso de este modifier, dado que todos los usuarios van a utilizar la misma función para almacenar y recuperar sus respectivos datos. Este es el encargado de aislar los datos vinculados a un usuario del resto.
...
**TrustYourFam** es la encargada de añadir una dirección dada a la lista personal con la que cuenta cada usuario de direcciones de confianza. La función devolverá un booleano que será "true" si se ha podido llamar exitosamente a la función approveSBT y "false" si no se cumple alguna de las condiciones especificadas.
...
Por otra parte está la función **deleteFam**. Esta función es la encargada de vaciar la lista de direcciones de confianza de un SBT dado. Para ello hace uso de la función predefinida pop. Esta requiere utilizar pop dado que interesa no sólo sobrescribir la posición de la lista sino acortarla para que no interfiera con el require de tres posiciones máximo definido en trustYourFam. 
...
Finalmente se cuenta con **amIFam**. Esta función se utiliza fundamentalmente para poder realizar comprobaciones en el contrato CajaFuerte, para ver si una dirección dada pertenece a la lista de direcciones de confianza de un token dado. 
...
Esta función es realmente una interfaz para llamar a la ya mencionada tokenOfOwnerByIndex(_owner, 0), obteniendo mediante los parámetros una forma de referenciar a la primera posición del diccionario _ownedTokens que hereda del estándar ERC721Enumerable, el cual almacena una lista indexada de los SBT de una dirección dada.
...
Como ya se ha descrito anteriormente, este modifier es el encargado de realizar una comprobación presente en trustYourFam y deleteFam. Para poder dar o eliminar permisos a una dirección dada es necesario que el dueño del SBT o aquellos terceros con confianza ya asignada la otorguen. 

Solo cuenta con un require en su interior que llama tanto a la función walletOfOwner, para comprobar si el SBT que tiene el dueño en su dirección coincide con aquel al que se pretende aplicar un tratamiento, como a la función amIFam, para verificar si la dirección que lanza la transacción cuenta con permisos sobre dicho token. 

Si se cumple alguna de las dos condiciones, se podrá ejecutar el resto de la función sobre la que este modifier aplique. Además, en caso de cumplirse la condición definida por walletOfOwner, es necesario también que el identificador del token sea distinto de cero. Esto se debe a una solución ante una posible vulnerabilidad por el funcionamiento interno de walletOfOwner, que en caso de no encontrar un SBT en la cartera de un usuario, devuelve un cero.

Incluye también la sintaxis “_;” utilizada para exigir que primero se procese la condición y posteriormente el código de las funciones en las que se aplica el modificador. 
