...
3. **Funciones recuperarRegistro(uint256 index, uint256 _soulBoundToken) y recuperarVault(uint256 _soulBoundToken)**: Ambas tienen el mismo objetivo, recuperar el o los registros almacenados a partir de una clave (el identificador tokenId de un SBT). La primera de ellas permite obtener uno en concreto. mientras que la segunda devuelve todos para una clave dada.
